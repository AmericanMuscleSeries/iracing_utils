# Started from https://github.com/KuzmaLesnoy/garage61api
...
class Garage61Client:
    def __init__(self,
        ...
        - use_garage61_ids: False by default. True if you provide Garage61 car and track IDs instead of iRacing ones.
        ...
        ids_file = Path("./g61.json")
        if ids_file.exists():
            with ids_file.open('rt') as f:
                self.ids = json.loads(f.read())
        else:
            self.ids = {"cars": self.cars(), "tracks": self.tracks()}
            # Let's convert platform id's from str to int
            #  TODO Could other platforms id's not be ints?
            for car in self.ids["cars"]:
                if isinstance(car["platform_id"], str):
                    car["platform_id"] = int(car["platform_id"])
            for track in self.ids["tracks"]:
                if isinstance(track["platform_id"], str):
                    track["platform_id"] = int(track["platform_id"])
    ...
    def _ids_converter(self,
        ...
        def _get_car_id(iracing_car_id: int) -> int:
            ...
            if iracing_car_id < 0:
                return iracing_car_id
            for car in self.ids['cars']:
                if car['platform_id'] == iracing_car_id:
                    return car['id']
    ...
    def refresh_token(self,
        ...
        Your application should check the validity of the Access Token itself and, when it expires,
    ...
    def me_statistics(self,
        ...
        Each returned record is a combination of car, track and day.
    ...
    def teams(self,
        ...
        The member list of a team might not be included if the authenticated user is not a team owner
        and the team is configured to only show the member list to team owners.

        :param team_id: team ID or Slug to get information about a specific team.
        :param team_statistics: team_id is required! Get team driving statistics.
        Driving statistics for the members of the team.
        Each returned record is a combination of car, track, day and driver.
        You'll need to sum up these rows according to the specific numbers you want to calculate (e.g. per car).
    ...
    def car_groups(self) -> list[dict]:
        ...
        For simulator platforms such as iRacing, this returns all available car groups,
        not just the ones that are owned by the user.
    ...
    def cars(self) -> list[dict]:
        ...
        For simulator platforms such as iRacing, this returns all available cars,
        not just the ones that are owned by the user.
    ...
    def platforms(self) -> list[dict]:
        ...
        All data is linked to a platform, which can either be a simulator platform (such as iRacing)
        or the real world (identified by -).
    ...
    def tracks(self) -> list[dict]:
        ...
        For simulator platforms such as iRacing, this returns all available tracks,
        not just the ones that are owned by the user.
    ...
    def laps(self,
        ...
        The drivers selected by "drivers" are combined with those from the "teams" and "extra_drivers" parameters.
        If none of these parameters are given, yourself and all teammates will be selected.

        Limitations

        This method only returns laps driven yourself or your teammates.
        Use the relevant parameters to choose the desired selection of laps.
        For privacy reasons, no global lap search is available.
        Data returned adheres to the defined privacy settings and is thus specific to the user linked to the API token.

        :param lap_id: lap ID returns all info about a specific lap if provided. Ignoring other filters.
        Structure of response: https://garage61.net/developer/endpoints/v1/getLap
        :param cars: car IDs to search for (int or list of ints).
        :param teams: teams to include (by team slug) (str or list of str).
        Teammates from the given teams will be included in the results.
        :param tracks: track IDs to search for (int or list of ints).
        :param iracing_seasons: season IDs to search for (int or list of ints).
        ...
        :param age: maximum lap age. Any positive number represents the maximum number of days ago
        (e.g. 7 would restrict the search to all laps driven in the past week).
        A negative value is interpreted as follows:
          Current season: -1; Current and previous season: -2; Last 3 seasons: -3; Last 4 seasons: -4.
        :param date_after: laps driven after given date (datetime or str).
        :param session_types: filter by session type (default = all) (int or list of ints).
            Available values: 1: Practice; 2: Qualifying; 3: Race.
        :param setup_types: filter by session setup type (default = all) (int or list of ints).
            Available values: 1: Open setup; 2: Fixed setup.
        :param telemetry_required: require the telemetry of each lap to be visible (bool).
            Requires the calling user to have a Pro plan.
        :param ghost_lap_required: require the ghost lap of each lap to be visible (bool).
            Only selects laps that have a ghost lap available. Requires the calling user to have a Pro plan.
        :param setup_required: require the setup of each lap to be visible (bool).
            Only selects laps that have a setup available. Requires the calling user to have a Pro plan.
        :param lap_types: filter by lap type (default = normal laps only) (int or list of ints).
            Available values: 1: Normal (full) laps;
                              2: Joker laps;
        ...
        :param min_cond_cloud: minimum cloud cover (int or list of ints).
            Available values: 1: Clear skies; 2: Partly cloudy; 3: Mostly cloudy; 4: Overcast.
        :param max_cond_cloud: maximum cloud cover (int or list of ints).
            Available values: 1: Clear skies; 2: Partly cloudy; 3: Mostly cloudy; 4: Overcast.
        :param cond_wind_dir: allowed wind directions (int or list of ints).
            Available values: 1: North; 2: East; 3: South; 4: West.
        :param rounding: correct parameters for rounding (str).
            When using the web app, lap searches are adjusted slightly to compensate for the fact that values are
            rounded for display. Supply the desired display units to apply rounding correction on your lap search.
            Available values: "metric", "englishStandard".
        :param group: grouping of results, determines which laps will be returned (default: driver) (str).
            Available values: "driver": Personal best laps per driver;
                "driver-car": Personal best laps per driver/car combination; "none": Return all laps.
    ...
    def lap_csv(self,
        ...
        """
        Export the telemetry for a lap as a CSV file.
        Returns a CSV file with the telemetry for the requested lap (if available). Required permissions: driving_data
